...
wcsim_file_path = f'{PNFS_PERSISTENT}/output/genie_muons/thru_individual_tilts/'     # wcsim root files from the grid

# from previous jobs
'''
#events_per_job = 50    # match wcsim root jobs
N_jobs = 500           # total number of jobs sent / present
~25k total events possible to loop over for these jobs
'''
...
print('\nThere are', len(valid_files), 'total events in the WCSim file path to run over')
print('We will partition these into', files_per_job, 'event chunks')

# determine the number of jobs needed to run over all events
N_events = len(valid_files)
J = int(N_events / files_per_job)
print('   --> we will therefore submit', J, 'jobs')


print('\nSubmitting jobs...\n')

# Batch into J jobs
for job_id in range(J+1):
